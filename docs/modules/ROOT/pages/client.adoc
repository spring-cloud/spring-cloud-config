...
Retry is built on the retry support of Spring Framework, so no extra dependency is required.
...
`max-attempts` counts the initial invocation, so the default of `6` means one invocation followed by at most five retries.
You can configure these properties (and others) by setting the `spring.cloud.config.retry.*` configuration properties.
To turn retry off while keeping `fail-fast` enabled, set `spring.cloud.config.retry.enabled=false`; the client then contacts the config server exactly once.

NOTE: Prior to Spring Cloud Config 5.1, retry required Spring Retry, so an application that set `spring.cloud.config.fail-fast=true` without Spring Retry on its classpath contacted the config server only once.
Retry is now provided by Spring Framework, so the default retry configuration applies to such applications as well.
To keep the previous single-attempt behavior, set `spring.cloud.config.retry.enabled=false`.

NOTE: `spring.cloud.config.retry.useRandomPolicy` is deprecated and will be removed in a future release.
While it is `true`, the backoff interval is randomized upwards, into the range from the interval the deterministic
exponential backoff would have used up to that interval multiplied by `multiplier`, capped at `max-interval`.
The `max-attempts`, `initial-interval`, `max-interval`, and `multiplier` properties still have an effect.

TIP: To take full control of the retry behavior and are using legacy bootstrap, add a `@Bean` of type `MethodInterceptor` with an ID of `configServerRetryInterceptor`.
Prior to Spring Cloud Config 5.1 that bean had to be a Spring Retry `RetryOperationsInterceptor`; the bean name is unchanged.
...
To turn retry off for such an import while keeping `fail-fast` enabled, add `enabled=false`, which sets `spring.cloud.config.retry.enabled=false`:

[source,properties]
----
spring.config.import=configserver:http://configserver.example.com?fail-fast=true&enabled=false
----
...
The preceding code would set the value of the `name` variable to `appAsecret`.
