...
Then you need to enable retry by setting `spring.cloud.config.retry.enabled=true`.
Retry is built on the https://docs.spring.io/spring-framework/reference/core/resilience.html[retry support of Spring Framework], so no extra dependency is required.
Once enabled, the default behavior is to retry six times with an initial backoff interval of 1000ms and an exponential multiplier of 1.1 for subsequent backoffs.
...
NOTE: Prior to Spring Cloud Config 5.1, retry was enabled by adding Spring Retry to the classpath.
Spring Retry is no longer used, so set `spring.cloud.config.retry.enabled=true` to keep retrying after upgrading.
...
This sets `spring.cloud.config.fail-fast=true` (notice the missing prefix above) and the supported retry properties shown in the URL above.
