...
The repository implementation maps the `\{label}` parameter of the HTTP resource to https://docs.aws.amazon.com/secretsmanager/latest/userguide/whats-in-a-secret.html#term_version[AWS Secrets Manager secret's staging label]. To create a labelled secret, create a secret or update its content and define a staging label for it (sometimes it's called version stage in the AWS documentation). For example:
...
Note that if the staging label contains a slash (`/`), then the label in the HTTP URL should instead be specified with the special string `({special-string})` (to avoid ambiguity with other URL paths) the same way xref:server/environment-repository/git-backend.adoc[Git backend's section] describes it.
