...
NOTE: The "`one-liner`" shortcut used in the "`simple`" example can be used only if the only property to be set is the URI.
...
If you provide a username and password with an AWS CodeCommit URI, they must be the https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_access-keys.html[AWS accessKeyId and secretAccessKey] that provide access to the repository.
...
Spring Cloud Config Server also supports authenticating against https://dev.azure.com[Azure DevOps] Git repositories using Azure Managed Identity.

When your Git URI uses `http` or `https` and targets Azure DevOps (host `dev.azure.com`), you can enable managed identity authentication by setting `spring.cloud.config.server.git.azure.identity.managed-identity-enabled` to `true`. Spring Cloud Config Server will automatically acquire an Azure AD access token for Azure DevOps and configure HTTP Bearer token authentication on the JGit transport.
        ...
        git:
          ...
          azure:
            identity:
              ...
              client-id: 00000000-0000-0000-0000-000000000000 # optional, for user-assigned managed identity
----

If using a user-assigned managed identity, specify the client ID using `spring.cloud.config.server.git.azure.identity.client-id`. For system-assigned managed identity, `client-id` can be omitted.

The property can also be configured per-repository when using multiple repositories:
...
NOTE: `com.azure:azure-identity` is an optional dependency.
If the `azure-identity` jar is not on your classpath, the Azure DevOps managed identity credentials provider will not be created, regardless of the Git server URI.
