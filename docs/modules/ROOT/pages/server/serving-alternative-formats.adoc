...
By default, YAML and JSON placeholder resolution fails when a nested placeholder cannot be resolved.
To preserve unresolved nested placeholders in these responses instead, set `spring.cloud.config.server.ignore-unresolvable-nested-placeholders` to `true` in the Config Server configuration.
