...
|spring.cloud.config.retry.enabled | `+++true+++` | Whether to retry failed requests to the Config Server. Only has an effect when spring.cloud.config.fail-fast is enabled.
