...
The Config Server properties show up in the `actuator/env` endpoint as a high-priority property source, as shown in the following example.
...
$ curl localhost:8080/actuator/env
{
  ...
  ....
  {
      "name": "bootstrapProperties-https://github.com/spring-cloud-samples/config-repo/foo.properties",
      "properties": {
        "foo": {
