...
If you have multiple instances of Config Server deployed and expect instance(s) to be unavailable from time to time , to ensure high availability , you can specify multiple urls , comma-separated , under the property - `spring.cloud.config.uri`. Note , this will ensure high availability only in case if the Config Server is not running (application has exited) or a connection time-out occurs. For example - If the Config Server returns a 500 (Internal Server Error) response or the Config Client receives a 401 from the Config Server due to bad credentials etc, the Config Client will NOT attempt to fetch properties from other urls (if mentioned). The idea behind this been , any error of that kind indicates an issue with your security configuration on the client/server side , yaml property files being in invalid format in your git repository etc. Hence , not an 'availability' problem and a problem which requires attention from the user.

If you are using HTTP Basic Security on your Config Server(s) , currently it is possible to support per Config Server auth credentials ONLY if you embed credentials in each url you specify under the property - `spring.cloud.config.uri`. If you are using any other kind of Security mechanism , it is NOT possible currently to support per Config Server authentication/authorization.  
