...
The #_property_overrides[property overrides] feature can also be used for setting global defaults, with placeholders applications
...
If, for example, you want to pull configuration data from a Git repository as well as an SVN repository, you can set the following properties for your configuration server:
...
In addition to each repository specifying a URI, you can also specify an `order` property.
The `order` property lets you specify the priority order for all your repositories.
...
If you have an environment similar to the one in the preceding example and you request configuration data with the `master` label but the SVN repository does not contain a branch called `master`, the entire request fails.
...
The `spring.cloud.config.authorization` property has now been removed. It can now be provided through the additional headers used to create the client request - `spring.cloud.config.headers.authorization`. Also , two new properties have been added - `spring.cloud.config.requestReadTimeout` and `spring.cloud.config.sendState`. Please find below the complete list of client properties along with their description.

[options="header"]
|===
^|Property ^|Description
|`spring.cloud.config.enabled`|Flag to say that remote configuration is enabled. Default true.
|`spring.cloud.config.profile`|The default profile to use when fetching remote configuration (comma-separated).Default is 'default'.
...
|`spring.cloud.config.uri`|The URI of the remote server (comma-separated).
|`spring.cloud.config.failFast`|Flag to indicate that failure to connect to the server is fatal (default false).
|`spring.cloud.config.token`|Security Token passed through to underlying environment repository.
|`spring.cloud.config.requestReadTimeout`|timeout on waiting to read data from the Config Server.
|`spring.cloud.config.sendState`|Flag to indicate whether to send state. Default true.
|`spring.cloud.config.headers.*`|Additional headers used to create the client request.
|`spring.cloud.config.discovery.enabled`|Flag to indicate that config server discovery is enabled (config server URL will be looked up via discovery). Default is False.
|`spring.cloud.config.discovery.serviceId`|Service id to locate config server. Default is 'configServer'.
|===
