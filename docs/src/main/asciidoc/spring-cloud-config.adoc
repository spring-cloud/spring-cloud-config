...
By default, the database needs to have a table called `PROPERTIES` with columns called `APPLICATION`, `PROFILE`, and `LABEL` (with the usual `Environment` meaning), plus `CONFIG_KEY` and `CONFIG_VALUE` for the key and value pairs in `Properties` style.
...
TIP: If you want to use customized table name, or customized column name, or even customized table structure, you may use `spring.cloud.config.server.jdbc.sql` configuration to define a customized sql statement.
     The default value for this sql statement is `SELECT CONFIG_KEY, CONFIG_VALUE from PROPERTIES where APPLICATION=? and PROFILE=? and LABEL=?`.
