...
=== Configuring Proxy Connection for Config Client

If you want to configure a `proxy only for Config Client (*not for global enviroment*)` to connect Config Server.

You can specify the following properties:

* "host" = Proxy server host
* "port" = Proxy server port
* "type" = Proxy protocol: http(default) or socks
