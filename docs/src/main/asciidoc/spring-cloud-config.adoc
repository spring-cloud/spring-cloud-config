...
If a client is setup to `failFast`, it can also be configured to retry contacting the Config Server with the addition of dependencies to Spring Retry and Spring Boot AOP. For Maven this would look like:
...
With these dependencies added, the retry functionality can be controlled by bootstrap properties under `spring.config.retry`. The available properties are:

* `initialInterval` initial timeout in mS (default 1000)
* `multiplier` multiplier for subsequent timeouts (default 1.1)
* `maxInterval` maximum timeout in mS (default 2000)
* `maxAttempts` maximum number of retries (default 6)

An example bootstrap yml would look like:
