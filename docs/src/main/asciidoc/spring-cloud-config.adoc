...
Many source code repository providers (such as Github, Gitlab, Gitee, Gogs, or Bitbucket) notify you of changes in a repository through a webhook.
...
In addition to the JSON notifications from Github, Gitlab, Gitee, Gogs, or Bitbucket, you can trigger a change notification by POSTing to `/monitor` with form-encoded body parameters in the pattern of `path={name}`.
