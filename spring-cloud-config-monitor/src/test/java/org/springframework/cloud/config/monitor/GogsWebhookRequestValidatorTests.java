...
class GogsWebhookRequestValidatorTests {
	...
	private final GogsWebhookRequestValidator validator = new GogsWebhookRequestValidator(SECRET);
	...
	void shouldValidate_pushEvent() {
		...
		req.addHeader("X-Gogs-Event", "push");
	...
	void shouldValidate_otherEvent() {
		MockHttpServletRequest req = new MockHttpServletRequest();
		req.addHeader("X-Gogs-Event", "issues");
	...
	void validate_validSignature() throws Exception {
		...
		MockHttpServletRequest req = new MockHttpServletRequest();
		req.setContent(BODY);
		req.addHeader("X-Gogs-Signature", signature);
	...
	void validate_invalidSignature() {
		MockHttpServletRequest req = new MockHttpServletRequest();
		req.setContent(BODY);
		req.addHeader("X-Gogs-Signature", "deadbeef");
