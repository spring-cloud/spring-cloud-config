...
class EnvironmentControllerIntegrationTests {
	...
	abstract static class TestCases {
		...
		public void environmentNoLabel() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").build();
		...
		public void profileWithDash() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("dev-db").build();
			Environment dashEnvironment = new Environment("foo", "dev-db");
			dashEnvironment.add(new PropertySource("foo", new HashMap<>()));
			when(this.repository.findOne(ctx)).thenReturn(dashEnvironment);
		...
		public void profileContainingExtensionKeyword(String extensionKeyword) throws Exception {
			...
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles(profiles).build();
		...
		public void propertiesLabel() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("label").build();
			when(this.repository.findOne(ctx)).thenReturn(this.environment);
		...
		public void propertiesLabelWhenApplicationNameContainsHyphen() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo-bar").profiles("default").label("label")
					.build();
		...
		public void propertiesLabelWithSlash() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("label/spam")
					.build();
			when(this.repository.findOne(ctx)).thenReturn(this.environment);
		...
		public void environmentWithLabel() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("awesome").build();
			when(this.repository.findOne(ctx)).thenReturn(this.environment);
		...
		public void environmentWithMissingLabel() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("missing").build();
			when(this.repository.findOne(ctx)).thenThrow(new NoSuchLabelException("Planned"));
		...
		public void environmentWithMissingRepo() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("missing").build();
			when(this.repository.findOne(ctx)).thenThrow(new NoSuchRepositoryException("Planned"));
			this.mvc.perform(MockMvcRequestBuilders.get("/foo/default/missing"))
					.andExpect(MockMvcResultMatchers.status().isNotFound());
		}
		...
		public void environmentWithLabelContainingPeriod() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("1.0.0").build();
			when(this.repository.findOne(ctx)).thenReturn(this.environment);
		...
		public void environmentWithLabelContainingSlash() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo").profiles("default").label("feature/puff")
					.build();
			when(this.repository.findOne(ctx)).thenReturn(this.environment);
		...
		public void environmentWithApplicationContainingSlash() throws Exception {
			RequestContext ctx = new RequestContext.Builder().name("foo/app").profiles("default").build();
			Environment environment = new Environment("foo/app", "default");
			environment.add(new PropertySource("foo", new HashMap<>()));
			when(this.repository.findOne(ctx)).thenReturn(environment);
